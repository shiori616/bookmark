  ...
  function setupButtonEvents() {
    ...
    $(".read-btn").on("click", function () {
      ...
      const bookData = {
        ...
        buttonType: "read", // ボタンの識別
        clickDateTime: clickDateTime, // クリック日時
        status: "read",
      }

      console.log("Book data with industry identifiers:", bookData) // デバッグ用
      saveBookToDB(bookData, $(this))
    })
      ...
      success: (response) => {
        ...
        try {
          ...
          if (result.success) {
            ...
            alert(
              `「${bookData.title}」を${bookData.buttonType === "read" ? "読了済み" : "積読リスト"}に保存しました！`,
            )
          } else {
